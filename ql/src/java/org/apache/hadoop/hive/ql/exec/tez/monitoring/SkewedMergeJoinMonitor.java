...
public class SkewedMergeJoinMonitor implements SkewedJoinMonitor {
    ...
    @Override
    public void checkMergeJoinSkew(byte alias, long rowCount, String joinKeyColumns,
        String tableAlias)
    ...
    public static SkewedJoinMonitor createSkewedJoinMonitor(long mergeJoinSkewThreshold,
        boolean mergeJoinSkewAbort, long mergeJoinSkewCheckInterval, int maxAlias) {
        if (mergeJoinSkewThreshold > 0) {
            return new SkewedMergeJoinMonitor(mergeJoinSkewThreshold, mergeJoinSkewAbort,
                mergeJoinSkewCheckInterval, maxAlias);
        } else {
